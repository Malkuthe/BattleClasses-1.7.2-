...
public class PlayerClass implements IExtendedEntityProperties {
	...
	public static void loadProxyData( EntityPlayer player ){
		...
		BCMPacketHandler.INSTANCE.sendTo(new BCMClassMessage(player), (EntityPlayerMP) player);;
	}
	...
	public void ClassChange( String bcm ){
		if ( this.playerClass.equals(Configs.defaultClass) && !bcm.equals(Configs.defaultClass)) {
			...
			player.addChatMessage( new ChatComponentText( Configs.noviceJoin + EnumChatFormatting.RED + this.playerClass + EnumChatFormatting.RESET + Configs.noviceJoinCont ));
			
			//debug
			System.out.println("[PlayerClasses] class chosen confirmed");
		} else if( this.playerClass.equals(bcm) ){
			player.addChatMessage( new ChatComponentText( Configs.classChangeErr + EnumChatFormatting.RED + this.playerClass + EnumChatFormatting.RESET + Configs.classChangeErrCont));
		} else {
			this.playerClass = bcm;
			player.addChatMessage( new ChatComponentText( Configs.classChange + EnumChatFormatting.RED + this.playerClass + EnumChatFormatting.RESET + Configs.classChangeCont));
	...
	public void ClassChangeDiff( EntityPlayer commandplayer, EntityPlayer player, String bcmclass ){
		...
		if ( this.playerClass.equals(bcmclass) ){
			player.addChatMessage(new ChatComponentText( EnumChatFormatting.GREEN + playername + EnumChatFormatting.RESET + " is already a " + EnumChatFormatting.RED + this.playerClass));
		} else {
			String orgClass = this.playerClass;
			this.playerClass = bcmclass;
			player.addChatMessage( new ChatComponentText( EnumChatFormatting.GREEN + "[SERVER]" + EnumChatFormatting.RESET + "Your class has been changed from " + EnumChatFormatting.RED + orgClass + EnumChatFormatting.RESET + " to " + EnumChatFormatting.RED + this.playerClass ));
			commandplayer.addChatMessage( new ChatComponentText( EnumChatFormatting.GREEN + playername + "'s " + EnumChatFormatting.RESET + "class has been changed from " + EnumChatFormatting.RED + orgClass + EnumChatFormatting.RESET + " to " + EnumChatFormatting.RED + this.playerClass ));
		}
	}
	
	public void Demote( EntityPlayer player ){
		if (this.playerClass.equals(Configs.defaultClass)){
			player.addChatMessage( new ChatComponentText( "You are already a " + EnumChatFormatting.RED + Configs.defaultClass));
		} else {
			this.playerClass = Configs.defaultClass;
			player.addChatMessage( new ChatComponentText( "You have been successfully demoted to " + EnumChatFormatting.RED + Configs.defaultClass));
		}
	}
	...
	public void DemoteDiff( EntityPlayer commandplayer, EntityPlayer player){
		String playername = ((EntityPlayer) player).getCommandSenderName();
		if (this.playerClass.equals(Configs.defaultClass)){
			commandplayer.addChatMessage( new ChatComponentText( EnumChatFormatting.GREEN + playername + EnumChatFormatting.RESET + " is already a " + EnumChatFormatting.RED + Configs.defaultClass));
		} else {
			String orgclass = this.playerClass;
			this.playerClass = Configs.defaultClass;
			commandplayer.addChatMessage(new ChatComponentText( EnumChatFormatting.GREEN + playername + EnumChatFormatting.RESET + " has been successfully demoted from " + EnumChatFormatting.RED + orgclass + EnumChatFormatting.RESET + " to " + EnumChatFormatting.RED + Configs.defaultClass));
			player.addChatMessage( new ChatComponentText( EnumChatFormatting.GREEN + "[SERVER]" + EnumChatFormatting.RESET + "You have been demoted from " + EnumChatFormatting.RED + orgclass + EnumChatFormatting.RESET + " to " + EnumChatFormatting.RED + Configs.defaultClass));
		}
	}
	
	public void ClassCheck(){
		if ( this.playerClass.equals(Configs.defaultClass) ) {
			player.addChatMessage( new ChatComponentText( Configs.classless + EnumChatFormatting.RED + this.playerClass + EnumChatFormatting.RESET + Configs.classlessCont));
		} else {
			player.addChatMessage( new ChatComponentText( Configs.classCheck + EnumChatFormatting.RED + this.playerClass + EnumChatFormatting.RESET + Configs.classCheckCont));
		}
	}
